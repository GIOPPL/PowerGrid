...
class ControlAdapt(mList: ArrayList<ControlEquipmentBean>, activity: Activity) : RecyclerView.Adapter<ControlAdapt.MyViewHolder>() {
    ...
    inner class MyViewHolder(itemView: View?) : RecyclerView.ViewHolder(itemView) {
        ...
        private fun initRV(itemView: View?) {
            ...
            val linearManager = LinearLayoutManager(itemView.context, LinearLayoutManager.HORIZONTAL, false)
            linearManager.isSmoothScrollbarEnabled = true
            mRV.layoutManager = linearManager
            mRV.adapter = mAdapt
        }
